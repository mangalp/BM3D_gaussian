...
def computeKLLoss(mu, logvar):
    ...
    """
    kl_level = -0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())
    return kl_level

def lossFunctionKLD(mus, logvars):
    kl_error = 0.0
    for mu, logvar in zip(mus, logvars):
        kl_error += computeKLLoss(mu, logvar)

    kl_error /= float(len(mus))
    return kl_error
...
def loss_fn(predicted_s, x, mus, logvars, gaussian_noise_std, data_mean, data_std, device, noiseModel):
    ...
    """
    kl_loss = lossFunctionKLD(mus, logvars)
